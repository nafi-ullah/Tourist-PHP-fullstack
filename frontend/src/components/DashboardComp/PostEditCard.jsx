...
const PostEditCard = ({postid, userid, headline, caption, picture, country}) => {
    const [inputs, setInputs] = useState({
        country: country || '',
        headline: headline || '',
        caption: caption || '',
        picture: picture || '' // Initialize picture as null
    });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            axios.put(`http://localhost/api/user/posts/${postid}`, inputsWithAction).then(function(response){
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-md min-w-[600px] w-full">
    <form onSubmit={handleSubmit}>
        {/* <!-- Post Content Section --> */}
        <div className="mb-6">
            <label htmlFor="headlineContent" className="block text-gray-700 text-sm font-bold mb-2">Headline</label>
            <textarea id="headlineContent" name="headline" rows="2"  value={inputs.headline} onChange={handleChange} className="w-full border-2 rounded-md px-4 py-2 leading-5 transition duration-150 ease-in-out sm:text-sm
    sm:leading-5 resize-none focus:outline-none focus:border-blue-500" placeholder="Headline"></textarea>
        ...
        <div className="mb-6">
            ...
            <textarea id="countryContent" name="country"  value={inputs.country} rows="1" onChange={handleChange} className="w-full border-2 rounded-md px-4 py-2 leading-5 transition duration-150 ease-in-out sm:text-sm
    sm:leading-5 resize-none focus:outline-none focus:border-blue-500" placeholder="Which Country?"></textarea>
        ...
        <div className="mb-6">
            ...
            <textarea id="postContent" name="caption" rows="4" value={inputs.caption} onChange={handleChange} className="w-full border-2 rounded-md px-4 py-2 leading-5 transition duration-150 ease-in-out sm:text-sm
